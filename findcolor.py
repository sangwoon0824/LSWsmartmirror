...
while(True):
    ...
    cv.drawContours(temp_result,contours[0],contourIdx=-1,color=(255,255,255))


    cv.imshow('img_color', img_color)
    cv.imshow('img_mask', img_mask)
    cv.imshow('img_thresh', img_thresh)
    sub = cv.subtract(img_color, img_result)
    cv.imshow('img_result', sub)
    cv.imshow('temp_result', temp_result)
